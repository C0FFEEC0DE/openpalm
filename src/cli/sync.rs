...
pub async fn sync_device(socket: &mut PilotSocket) -> Result<()> {
    ...
    for db in &dbs {
        ...
        let handle = socket
            .open_database(&db.name, crate::protocol::dlp::DlpOpenMode::Read)
            .await?;
        let mut modified_count = 0u32;

        // Try to read next modified record
        while let Ok(Some(_record)) = socket
            .dlp()
            .ok_or(PilotError::DlpSocket)?
            .read_next_modified_rec(handle)
            .await
        {
            modified_count += 1;
        }

        rows.push(vec![db.name.clone(), format!("{}", modified_count)]);

        socket.close_database(handle).await?;
    }

    print_table(&["Database", "Modified Records"], &rows);

    println!("Closing conduit...");
    socket
        .dlp()
        .ok_or(PilotError::DlpSocket)?
        .end_sync(crate::protocol::dlp::DlpEndStatus::Normal)
        .await?;
